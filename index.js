...
async function mailConfirm(id) {
	const res = await axios.get(
		`http://local.adspower.net:50325/api/v1/browser/start?user_id=${id}&launch_args=["--start-fullscreen","--disable-web-security","--disable-features=IsolateOrigins,site-per-process"]`
	);
	...
	if (res.data.code === 0 && res.data.data.ws && res.data.data.ws.puppeteer) {
		...
		console.log(
			`${colors.blue("Подтверждаю аккаунт: ")} ${colors.magenta(userData.name)}`
		);
		...
		try {
			...
			const mailChangeLink = await iframe.waitForXPath(
				`//div[@class="ptm"]/div/div/a`
			);
			await mailChangeLink.click();

			const newMailInput = await iframe.waitForXPath(
				`//input[@name="new_email"]`
			);
			await newMailInput.type(emailAddress, { delay: 10 });

			await page.waitForTimeout(2000);
			const submitFormButton = await iframe.waitForXPath(
				`(//button[@type="submit"])[2]`,
				{ visible: true }
			);
			await submitFormButton.click({ delay: 10 });

			const accountPasswordConfirm = await iframe.waitForXPath(
				`//input[@id="ajax_password"]`
			);
			await accountPasswordConfirm.type(accountPassword, { delay: 10 });

			const accountPasswordConfirmBtn = await iframe.waitForXPath(
				`//button[@data-testid="sec_ac_button"]`
			);
			...
			const confirmButton = await iframe.waitForXPath(
				`//a[@class="_42ft _42fu SettingsEmailPendingConfirm"]`
			);
			...
			const codeSubmitButton = await iframe.waitForXPath(
				`//button[text()="ОК"]`
			);
			...
			await adspower.addAccountRemark(
				id,
				`${userData.remark}|${emailAddress}|${emailPassword}`
			);
			console.log(
				`${colors.green("REMARK: ")} ${colors.cyan(
					`${userData.remark}|${emailAddress}|${emailPassword}`
				)}`
			);
			...
			mailData.shift();
			fs.writeFileSync(
				path.resolve(__dirname, "src", "data.txt"),
				mailData.join("\r\n")
			);
			...
			try {
				await page.waitForXPath(
					`//span[contains(text(),"ваш аккаунт заблокирован") or contains(text(),"Мы временно заблокировали ваш аккаунт")]`,
					{ timeout: 15000 }
				);
				return {
					status: "CHECK",
				...
				console.log(
					`${colors.red("Что то пошло не так: ")} ${colors.blue(err)}`
				);
			}
		}
	}
	...
	for (const id of idList) {
		try {
			...
			if (mailConfirmResult.status === "OK") {
				console.log(
					`${colors.red(
						`Переименовал аккаунт как ${mailConfirmResult.userData.name} EMAIL_CONFIRM`
					)}`
				);
				await delay(2000);
				await adspower.accountRename(
					id,
					`${mailConfirmResult.userData.name} EMAIL_CONFIRM`
				);
				await delay(3000);
			...
			if (mailConfirmResult.status === "NO CODE") {
				console.log(
					`${colors.red("Не получилось получить код подтверждения")}`
				);
				await mailConfirmResult.browser.close();
			}
			...
			if (mailConfirmResult.status === "CHECK") {
				console.log(
					`${colors.red(
						`Аккаунт улетел на чек! Переименую как ${mailConfirmResult.userData.name} CHECK`
					)}`
				);
				await adspower.accountRename(
					id,
					`${mailConfirmResult.userData.name} CHECK`
				);
				idList = idList.filter((item) => item !== id);
				fs.writeFileSync("./src/id.txt", idList.join("\r\n"));
				await mailConfirmResult.browser.close();
